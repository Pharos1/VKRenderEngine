...
int main() {
	...
	if (strcmp(f.substr(f.find_last_of("\\") + 1).c_str(), "FirstVulkanProject") == 0) {
...
void createGraphicsPipeline() {
	auto vertShaderCode = readFile("Shaders/spv/main.vert.spv");
	auto fragShaderCode = readFile("Shaders/spv/main.frag.spv");
...
void createFramebuffers() {
	...
	for (size_t i = 0; i < swapChainImageViews.size(); i++) {
		...
		if (msaaSamples != VK_SAMPLE_COUNT_1_BIT) {
			...
			attachments.resize(2);
			attachments = { depthImageView, swapChainImageViews[i] };
		}
...
void updateUniformBuffer(uint32_t currentImage) {
	...
	ubo.model = glm::mat4(1.f);// glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	ubo.proj = glm::perspective(glm::radians(45.0f), swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 100.0f);
	ubo.proj[1][1] *= -1; //Cuz we are working in left hand coord space
